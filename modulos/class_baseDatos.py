...
class BaseDatos(object):
	...
	# Variables:
	#			day_count: cantidad de dias a registrar
	def __init__(self,Ndays=5):
			self.day_count = Ndays
	
	
	# Almacenamiento de mediciones recibidas
	# Variables: 
	#		values: valor medido
	#		sensor_code: IdSensor
	#		unit: unidad de la medicion"""
	def storeData(self, values, sensor_code, unit):
	...
	def createFile(self,sensor_code):
			...
			path = "/tmp"+'/'+sensor_code # Cambiado 13-11 porque /var/tmp mantiene los archivos despues de reiniciar
			# Tambíen se puede usar en /dev/shm
			...
			if logCounter >= self.day_count: # Modificado 13/11
				# 86400 = 1 dia
	...
	def storeDone(self):
		...
		'''
		Estaria bueno completarlo para saber que sensor guardo el dato, con que valor y si el mismo
		se ha guardado correctamente o no. Seŕia como un registro de eventos
		'''
	def getDayForStore(self):
		return self.day_count
	
	def setDayForStore(self,days):
		self.day_count = days

	def getReport(self):
		'''
		Metodo para solicitarle las últimas mediciones de cada archivo.
		'''
